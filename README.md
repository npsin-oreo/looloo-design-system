...
```mermaid
flowchart LR
    A["brand.config.json<br/>brands/&lt;name&gt;.config.json<br/><i>brand overlay config</i>"] -->|"npm run tokens:theme"| B
    B["tokens/<br/><i>hand-authored DTCG layer</i><br/>primitive → semantic →<br/>component → mode → theme"]
...
```bash
npm run tokens:theme     # brand.config + brands/*.config.json → tokens/theme/<brand>.json
npm run tokens:build     # tokens/ → dist/tokens/*.css (+ .ts + contract)
npm run tokens:validate  # โครงสร้าง token ถูกต้อง · ทุก ref resolve
npm run tokens:css-check # dist ตรง byte-identical กับ golden snapshot (gate ที่ CI บังคับ)
...
├── brand.config.json         ← 🎨 neutral white-label config (tokens:theme input)
├── brands/<name>.config.json ← 🎨 brand overlay config (teal ฯลฯ)
│
├── tokens/                   ← canonical hand-authored DTCG layer (primitive→semantic→component→mode→theme)
├── scripts/tokens/           ← สคริปต์ build / validate / audit / theme
...
| `npm run check` | ✅ gate ครบชุดในเครื่อง — typecheck + token validate/build/css-check + audit (styles/icons/contrast) แบบ `--strict` |
| `npm run tokens:build` | regenerate `dist/tokens/*.css` จาก `tokens/` |
| `npm run tokens:theme` | regenerate `tokens/theme/<brand>.json` จาก brand config(s) |
| `npm run build` | build production ของแอป |

**เปลี่ยนสี brand:** แก้ `brand.config.json` (neutral) หรือ `brands/<name>.config.json` → `npm run tokens:theme && npm run tokens:build` (อย่าแก้ CSS ที่ generate เอง) · ฝั่ง consumer ของ package: override CSS variable ตาม [`THEMING.md`](./THEMING.md)
...
ค่าเริ่มต้นเป็น neutral white-label — override สี brand ได้โดย **ไม่ต้อง fork และไม่ต้อง build** (v2.0.0: `ds-brand-build` ถูกลบแล้ว) แค่ override CSS variable ทับ:

```ts
import "@npsin-oreo/design-system/styles.css"
import "./app/brand.css"     // :root { --primary: … } โหลดทับให้ override ชนะ
```

รายชื่อ CSS variable ที่ theme ได้ดูจาก `dist/tokens/semantic.css` หรือ [`brand.schema.json`](./brand.schema.json) · วิธี + ตัวอย่างเต็มดู [`THEMING.md`](./THEMING.md)
...
| **CSS byte-identity** | `dist/tokens` ตรงกับ golden snapshot (`tokens:css-check`) |
